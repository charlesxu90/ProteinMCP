...
python src/create_mcp.py --github-url https://github.com/chaidiscovery/chai-lab --mcp-dir tool-mcps/chai1_mcp --use-case-filter 'structure prediction given a sequence with chai-1, structure prediction given a config file with chai-1, batch prediction with chai-1'
...
pmcp create --local-repo-path  /home/xux/Downloads/gromacs-2025.4/ --mcp-dir tool-mcps/gromacs_mcp --use-case-filter 'Regular MD simulations, RMSD and RMSF analysis, binding affinity analysis, FEP'
...
pmcp create --local-repo-path tool-mcps/mutcompute_mcp/repo/mutcompute/ --mcp-dir tool-mcps/mutcompute_mcp --use-case-filter 'Predicting the effect of point mutations on protein stability and function, the demo code to run is in tool-mcps/mutcompute_mcp/repo/mutcompute/run_prediction.sh'
